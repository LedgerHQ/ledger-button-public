...
<Callout type="warning">
  **v2 is coming soon.** This page documents the upcoming Solana supported
  multi-blockchain API. For the current stable release, see [v1](../get-started/v1).
</Callout>
...
Initialize once at application startup (see [Get started](../get-started/v2) and [Configuration](../configuration)):
...
Unlike EVM, there is no discovery event to dispatch: the Ledger wallet announces itself through the Wallet Standard as soon as `initializeLedgerProvider` runs. You only need to list the registered wallets.

<Callout type="warning">
  Never dispatch `wallet-standard:app-ready` yourself. That event carries the
  app's `register` callback in its `detail`, so emitting a bare event throws
  inside every listening wallet. Let `getWallets()` or your wallet adapter emit
  it for you.
</Callout>
...
Confirm `initializeLedgerProvider` ran in the browser before you query wallets, that you passed `solanaBlockchainProviderFactory`, and that Solana is enabled for your `dAppIdentifier`. On unsupported platforms (no Web HID / Web Bluetooth), initialization is a no-op and no Solana wallet is registered — see [Requirements](../requirements).
...
- [Get started](../get-started/v2) — install family packages and pass `blockchainProviderFactories`
