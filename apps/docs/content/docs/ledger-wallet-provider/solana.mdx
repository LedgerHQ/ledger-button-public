...
Ledger Wallet Provider registers a Solana wallet automatically when you call `initializeLedgerProvider`. The same initialization used for EVM (EIP-6963) also announces Ledger on Solana — no Solana-specific setup call is required.
...
- [Get started](./get-started) — install and initialize for EVM and Solana
