...
Ledger Wallet Provider registers a Solana Wallet Standard wallet when you call `initializeLedgerProvider` **and** Solana is enabled in your partner dApp configuration (`dAppIdentifier`). No extra Solana-specific setup call is required beyond init.

<Callout type="info">
  Complete [Get started](./get-started) first so `initializeLedgerProvider` runs in the browser (use a dynamic import in SSR frameworks). Confirm with Ledger that Solana is enabled for your dApp, then follow this page for Wallet Standard discovery and signing.
</Callout>
...
<Callout type="info">
  **Partner configuration:** Which families are registered depends on your dApp config. Solana enabled → Wallet Standard. Ethereum enabled → EIP-6963 / EIP-1193. Both enabled → both surfaces from the same `initializeLedgerProvider` call. Account pickers are filtered by the requesting family (Solana connect only lists Solana-compatible accounts).
</Callout>
...
Confirm `initializeLedgerProvider` ran in the browser before you query wallets, and that Solana is enabled for your `dAppIdentifier`. On unsupported platforms (no Web HID / Web Bluetooth), initialization is a no-op and no Solana wallet is registered — see [Requirements](./requirements).
