...
```typescript
{
  ...
  // Dev/debug options — not for production use
...
The blockchain families your dApp wants to enable. Each entry pairs a family identifier with its provider factory, sourced from the corresponding family package. Families not listed here are never loaded, regardless of what your partner configuration allows. **Available from v2+ only.** Not present in the v1 API.
