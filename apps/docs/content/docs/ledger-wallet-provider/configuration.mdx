...
```typescript
{
  ...
  blockchainProviderFactories: BlockchainProviderFactoryRegistration[]; // Blockchain families to register
...
### `environment` <DevBadge />

Target Ledger backend environment.

### `dmkConfig` <DevBadge />

Advanced escape hatch for overriding the underlying [Device Management Kit](https://github.com/LedgerHQ/device-sdk-ts) configuration — API base URLs, WebSocket endpoint, provider, etc. Leave unset unless Ledger has explicitly instructed you to override a specific field.

### `dmkLogLevel` <DevBadge />

Log verbosity of the underlying Device Management Kit, independent of `loggerLevel`. Accepted values: `'fatal'`, `'error'`, `'warn'`, `'info'`, `'debug'`. Default is `'error'`, so device-level logs stay quiet unless you opt in.

### `devConfig.stub` <DevBadge />
