...
Ledger Wallet Provider registers an EIP-1193 provider automatically when you call `initializeLedgerProvider`. The same initialization used for Solana also announces Ledger on EVM — no EVM-specific setup call is required.

<Callout type="info">
  Complete [Get started](./get-started) first so `initializeLedgerProvider` runs in the browser (use a dynamic import in SSR frameworks). Then follow this page for EIP-6963 discovery and signing.
</Callout>
...
<Callout type="info">
  **Multi-chain sessions:** One `initializeLedgerProvider` call registers both the EVM EIP-6963 provider and the Solana Wallet Standard wallet. Users can connect on either family from the Ledger UI. Account pickers are filtered by the requesting family (`eth_requestAccounts` only lists EVM-compatible accounts).
</Callout>
...
Call the function returned by `initializeLedgerProvider` when your application unmounts. Cleanup removes the Ledger UI and unregisters the EVM EIP-6963 listener (and the Solana Wallet Standard wallet).
...
Confirm `initializeLedgerProvider` ran in the browser before dispatching `eip6963:requestProvider`. On unsupported platforms (no Web HID / Web Bluetooth), initialization is a no-op and no provider is registered — see [Requirements](./requirements).
