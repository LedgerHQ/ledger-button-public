...
This page describes the public API of Ledger Wallet Provider: `initializeLedgerProvider` from `@ledgerhq/ledger-wallet-provider`, and `LedgerEIP1193Provider` (see [import path](#ledgereip1193provider) — it moved packages in v2).

---

## `initializeLedgerProvider(options)`

Initializes the Ledger Wallet Provider and injects the UI components into the DOM. From v2+, it registers blockchain providers for each factory you pass in `blockchainProviderFactories` that is also enabled in your partner dApp configuration (`dAppIdentifier`):

- EVM factory + Ethereum enabled → [EIP-6963](https://eips.ethereum.org/EIPS/eip-6963) / [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193)
- Solana factory + Solana enabled → [Wallet Standard](https://github.com/wallet-standard/wallet-standard)

Without factories, no chain provider is registered. In v1 there are no factories at all: the EVM provider is registered automatically and Solana is not supported — see the [migration guide](./migration). For chain-specific connect and signing examples, see [EVM](./blockchains/evm) and [Solana](./blockchains/solana).
...
type InitializeLedgerProviderOptions = {
  ...
  // Dev/debug - Intended for Ledger use only. Do not override, please keep the default values
  environment?: 'staging' | 'production';             // Target Ledger backend environment
  dmkConfig?: DeviceModuleOptions;                    // Override Device Management Kit endpoints
  dmkLogLevel?: 'fatal' | 'error' | 'warn' | 'info' | 'debug';  // default: 'error'
  devConfig?: {};                                     // Local stub modes
};
```

ℹ️ Note: `apiKey`, `dAppIdentifier`and`blockchainProviderFactories` are shown as required because the provider cannot authenticate or register a chain without them. Note that the TypeScript type still marks all three optional for backward compatibility, so the compiler will not catch a missing one.
...
<Callout type="info">
  If the browser does not support Web HID or Web Bluetooth,
  `initializeLedgerProvider` returns a no-op function without registering the
  provider or mounting any UI. See [Requirements](./requirements).
</Callout>
...
The class is exported so you can type the object you receive — it is not meant to be instantiated directly. Import it as a type from:

- v1 -> `@ledgerhq/ledger-wallet-provider`
- v2 -> `@ledgerhq/ledger-wallet-provider-evm`.

`initializeLedgerProvider` builds the instance for you and announces it through the `eip6963:announceProvider` event (see [EVM](./blockchains/evm)):
...
- [Get started](./get-started/v2) — install and initialize the provider
- [EVM](./blockchains/evm) — EVM Blockchain Provider
- [Solana](./blockchains/solana) — Solana Blockchain Provider
