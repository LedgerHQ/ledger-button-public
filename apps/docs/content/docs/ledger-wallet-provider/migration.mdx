...
The choice is yours: initialize the LedgerProvider with one or several family packages, for each blockchain you support. The SDK registers only the families that you opted for if they are enabled in your partner configuration.
...
`LedgerEIP1193Provider` is the type you apply to the provider object received from the `eip6963:announceProvider` event. Only the import changes in v2:
...
Add it as well if your dApp supports Solana:
