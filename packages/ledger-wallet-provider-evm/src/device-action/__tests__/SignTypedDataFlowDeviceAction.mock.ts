...
import type { SignTypedDataFlowDAInput } from "../SignTypedDataFlowDeviceActionTypes";
...
export const DEFAULT_TYPED_DATA: TypedData = {
  types: {
    EIP712Domain: [],
...
export const DEFAULT_INPUT: SignTypedDataFlowDAInput = {
  signType: "typed-message",
  derivationPath: "44'/60'/0'/0/0",
  typedData: DEFAULT_TYPED_DATA,
...
export function setupSignTypedDataMock(
  signature?: Signature,
  error?: unknown,
  childStep?: string,
): void {
  (SignTypedDataDeviceActionFactory as Mock).mockReturnValue({
    makeStateMachine: vi.fn().mockImplementation(() =>
      createMachine({
        initial: "pending",
        states: {
